...
export function Certifications() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      /* ── Seamless infinite loop — runs the OPPOSITE way to "Beyond the Code"
         (rightward) for a nice counter-motion. Two copies + a -50%→0 slide. */
      if (!reduce && trackRef.current) {
        loopRef.current = gsap.fromTo(trackRef.current,
          { xPercent: -50 },
          { xPercent: 0, ease: "none", duration: certifications.length * 3.6, repeat: -1 });
